<template>
  <div class="admin-banner">
    ...
    <base-table>
      ...
      <tbody>
        <tr v-for="(item, index) in data" :key="item.id">
          ...
          <td class="img">
            <div class="img">
              <img :src="`${baseURL}/uploads/catalog/${item.image_path}`" alt="" />
            </div>
          </td>
          <td>
            {{ item.title_tm }}
    ...
    <pop-up-catalog v-if="isPopUp" @close="closePopUp" :item="catalogItem"></pop-up-catalog>
  </div>
</template>

<script>
  import PopUpCatalog from '@/components/popup/PopUpCatalog.vue'
  import { GET_CATALOG, DELETE_CATALOG } from '@/api/admin.api'
  ...
  export default {
    ...
    methods: {
      ...
      async closePopUp() {
        this.isPopUp = false
        if (this.catalogItem) {
          Object.keys(this.catalogItem).forEach((key) => (this.catalogItem[key] = null))
        }
        await this.fetch()
      },
      openPopUp() {
