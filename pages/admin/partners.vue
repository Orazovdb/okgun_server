      ...
      <div class="partners__row">
        <div class="partners__item" v-for="item in data" :key="item.uuid">
          ...
          <div class="partners__item-image">
            <img
              :src="`${baseURL}/uploads/service/${item.image_path}`"
              alt=""
            />
          </div>
        </div>
...
<script>
import { DELETE_PARTNER, GET_PARTNERS } from "@/api/admin.api";
  ...
  methods: {
    ...
    async closePopUp() {
      ...
      if (this.partnersItem) {
        Object.keys(this.partnersItem).forEach(
          (key) => (this.partnersItem[key] = null)
        );
      }
    },
    async save() {
